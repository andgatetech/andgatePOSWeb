                    ...
                    <div className="flex h-16 justify-between">
                        ...
                        <div className="hidden items-center space-x-8 md:flex">
                            {menuItems.map((item, index) => (
                                <Link key={index} href={item.href} className="px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:text-blue-600">
                                    {item.label}
                                </Link>
                            ))}
                ...
                {isMenuOpen && (
                    <div className="border-t bg-white shadow-lg md:hidden">
                        <div className="space-y-1 px-2 pb-3 pt-2">
                            {menuItems.map((item, index) => (
                                <Link key={index} href={item.href} className="block px-3 py-2 text-sm font-medium text-gray-700 hover:text-blue-600" onClick={() => setIsMenuOpen(false)}>
                                    {item.label}
                                </Link>
                            ))}
                            <Link href="/login" className="block px-3 py-2 text-sm font-medium text-gray-700 hover:text-blue-600" onClick={() => setIsMenuOpen(false)}>
                                {t('layout.nav.login')}
                            </Link>
                            <Link href="/register" className="block px-3 py-2 text-sm font-medium text-blue-600 hover:text-blue-700" onClick={() => setIsMenuOpen(false)}>
                                {t('layout.nav.get_started')}
                            </Link>
                        </div>
                    </div>
