        ...
        <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                    <div className="rounded-lg bg-[#046ca9]/10 p-2 text-[#046ca9]">
                        <CreditCard className="h-5 w-5" />
                    </div>
                    <div className="min-w-0">
                        <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Package & Manual Payment</h2>
                        {isLoading ? (
                            <p className="mt-1 text-xs text-gray-500">Loading subscription status...</p>
                        ) : (
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                                <span>{subscription?.plan_name_en || 'No active package'}</span>
                                {subscription?.expire_date && <span>Expires {new Date(subscription.expire_date).toLocaleDateString('en-US')}</span>}
                                {summary?.remaining_days !== null && summary?.remaining_days !== undefined && <span>{summary.remaining_days} days left</span>}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <Link href="/manual-payments" className="inline-flex items-center justify-center rounded-lg bg-[#046ca9] px-4 py-2 text-sm font-semibold text-white hover:bg-[#035b8e]">
                        Submit / Renew Payment
                    </Link>
                </div>
            </div>
            ...
            {latest?.status === 'rejected' && latest?.rejected_reason && (
                <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700">
                    Rejected: {latest.rejected_reason}
                </div>
            )}
            {summary?.has_pending_payment && (
                <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700">
                    Payment submitted. Admin verification required before package activation.
                </div>
            )}
        </section>
    );
