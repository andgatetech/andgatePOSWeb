...
export default function BreakEvenPage() {
    ...
    const { data, isLoading, refetch } = useGetBreakEvenQuery(
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-xl font-bold text-gray-900">{t('lbl_break_even')}</h1>
                    <p className="text-sm text-gray-500">{t('lbl_break_even_desc')}</p>
                </div>
            </div>
                ...
                <>
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between"><span className="text-gray-500">{t('lbl_revenue')}</span><span className="font-medium">{formatCurrency(r?.revenue || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">{t('lbl_discounts')}</span><span className="font-medium">{formatCurrency(r?.discounts || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">{t('lbl_net_revenue')}</span><span className="font-medium">{formatCurrency(r?.net_revenue || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">{t('lbl_variable_costs')}</span><span className="font-medium">{formatCurrency(r?.variable_costs || 0)}</span></div>
                            <div className="flex justify-between border-t pt-2"><span className="text-gray-700">{t('lbl_contribution_margin')}</span><span className="font-bold">{formatCurrency(r?.contribution_margin || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">{t('lbl_contribution_margin_ratio')}</span><span className="font-medium">{((r?.contribution_margin_ratio || 0) * 100).toFixed(2)}%</span></div>
                        </div>
                    </div>
                </>
            )}
