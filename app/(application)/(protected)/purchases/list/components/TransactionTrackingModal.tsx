    ...
    return (
        <>
            ...
            <div className="fixed left-1/2 top-1/2 z-[61] w-full max-w-[95vw] -translate-x-1/2 -translate-y-1/2">
                <div className="mx-4 max-h-[95vh] overflow-y-auto rounded-lg bg-[#f6f8fb] shadow-lg">
                    {/* Header Card */}
                    <div className="rounded-t-2xl border border-slate-200 bg-white p-4 shadow-lg sm:p-6">
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div className="flex items-center gap-4">
                                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-blue-700 shadow-lg sm:h-16 sm:w-16">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500 sm:text-sm">
                                        {t('lbl_purchase_order')} #{purchaseOrder.id}
                                    </p>
                    ...
                    <div className="grid grid-cols-1 gap-4 p-4 sm:gap-6 sm:p-6 lg:grid-cols-3">
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-lg lg:col-span-2">
                            <h2 className="mb-6 flex items-center gap-2 text-xl font-bold text-gray-900">
                                <Wallet className="h-6 w-6 text-blue-600" />
                                {t('lbl_payment_summary')}
                            </h2>
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-700">{t('lbl_payment_progress')}</span>
                                ...
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
                                    <div className="rounded-lg border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 p-3 sm:p-4">
                                        <p className="mb-1 text-xs font-medium text-blue-700 sm:text-sm">{t('lbl_grand_total')}</p>
                                        <p className="text-xl font-bold text-blue-900 sm:text-2xl">{formatCurrency(purchaseOrder.grand_total)}</p>
                                    </div>
                                    <div className="rounded-lg border border-green-200 bg-gradient-to-br from-green-50 to-green-100 p-3 sm:p-4">
                                        <p className="mb-1 text-xs font-medium text-green-700 sm:text-sm">{t('lbl_amount_paid')}</p>
                                        <p className="text-xl font-bold text-green-900 sm:text-2xl">{formatCurrency(purchaseOrder.amount_paid)}</p>
                                    </div>
                                    <div className="rounded-lg border border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100 p-3 sm:p-4">
                                        <p className="mb-1 text-xs font-medium text-orange-700 sm:text-sm">{t('lbl_amount_due')}</p>
                                ...
                                <div>
                                    <h3 className="mb-4 flex items-center gap-2 text-lg font-bold text-gray-900">
                                        ...
                                        {t('lbl_transaction_history')}
                                    </h3>
                                    <div className="space-y-3">
                                        {transactions.length === 0 ? (
                                            <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-slate-50 py-12">
                                                <CreditCard className="mb-3 h-12 w-12 text-gray-400" />
                                                <p className="text-sm font-medium text-gray-600">{t('msg_no_transactions_recorded')}</p>
                                            </div>
                                        ) : (
                                            transactions.map((transaction: any) => (
                                                <div
                                                    key={transaction.id}
                                                    className="rounded-lg border border-slate-200 bg-gradient-to-r from-slate-50 to-slate-100 p-3 transition-all hover:shadow-md sm:p-4"
                                                >
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <p className="font-semibold text-gray-900">
                                                                    {t('lbl_payment')} #{transaction.id}
                                                                </p>
                                                                <p className="text-xs capitalize text-gray-500">
                                                                    {transaction.payment_method} {t('lbl_payment')}
                                                                </p>
                                                            </div>
                                                        </div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-lg">
                            <h2 className="mb-6 flex items-center gap-2 text-xl font-bold text-gray-900">
                                <Package className="h-6 w-6 text-[#046ca9]" />
                                {t('lbl_order_items')}
                            </h2>

                            <div className="space-y-4">
                                {items.map((item: any) => (
                                    <div key={item.id} className="rounded-lg border border-[#cde2ef] bg-[#eef7fc] p-4">
                                        <div className="flex items-start gap-3">
                                            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg border-2 border-[#cde2ef] bg-white">
                                                <Package className="h-6 w-6 text-[#046ca9]" />
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <h3 className="truncate font-bold text-gray-900">{item.product_name}</h3>
                                                <p className="text-sm font-medium text-[#046ca9]">{item.variant_name}</p>

                                                {item.variant_data && (
                                                    <div className="mt-2 flex gap-2">
                                                        {Object.entries(item.variant_data).map(([key, value]) => (
                                                            <span key={key} className="rounded-full border border-[#cde2ef] bg-white px-2 py-1 text-xs font-medium text-[#046ca9]">
                                                                {key}: {value as string}
                                                            </span>
                                                        ))}
                                                    </div>
                                                ...
                                                <div className="mt-3 space-y-2">
                                                    ...
                                                    <div className="flex justify-between text-sm">
                                                        <span className="text-gray-600">{t('lbl_price')}:</span>
                                                        <span className="font-bold text-gray-900">{formatCurrency(item.purchase_price)}</span>
                                                    </div>
                                                    <div className="border-t border-[#cde2ef] pt-2">
                                                        <div className="flex justify-between">
                                                            <span className="font-bold text-gray-700">{t('lbl_total')}:</span>
                                                            <span className="text-lg font-bold text-[#046ca9]">{formatCurrency(item.total)}</span>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    {/* Timeline */}
                    <div className="m-4 rounded-lg border border-slate-200 bg-white p-4 shadow-lg sm:m-6 sm:p-6">
                        <h2 className="mb-6 flex items-center gap-2 text-xl font-bold text-gray-900">
                            <Clock className="h-6 w-6 text-indigo-600" />
                            {t('lbl_order_timeline')}
                        </h2>
                            ...
                            <div className="space-y-6">
                                <div className="relative flex gap-4">
                                    ...
                                    <div className="flex-1 rounded-lg bg-gradient-to-r from-blue-50 to-transparent p-4">
                                        <h3 className="font-bold text-gray-900">{t('lbl_order_created')}</h3>
                                    ...
                                    <div key={transaction.id} className="relative flex gap-4">
                                        ...
                                        <div className="flex-1 rounded-lg bg-gradient-to-r from-green-50 to-transparent p-4">
                                            <h3 className="font-bold text-gray-900">
                                                {t('lbl_payment_received')} - {formatCurrency(transaction.amount)}
                                            </h3>
                                ...
                                <div className="relative flex gap-4">
                                    <div className="z-10 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-[#eef7fc] shadow-lg">
                                        <Clock className="h-6 w-6 text-white" />
                                    </div>
                                    <div className="flex-1 rounded-lg bg-[#eef7fc] p-4">
                                        <h3 className="font-bold text-gray-900">{t('lbl_last_updated')}</h3>
                                        <div className="text-sm text-gray-600">
