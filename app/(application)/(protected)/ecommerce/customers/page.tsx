...
// ── Customer Row Actions Dropdown ──
interface EcommerceCustomerActionsProps {
    customer: any;
    onQuickView: (customerId: number) => void;
}

const EcommerceCustomerActions: React.FC<EcommerceCustomerActionsProps> = ({ customer, onQuickView }) => {
        ...
        <div className="relative flex justify-end">
            ...
            {isOpen && (
                <>
                    ...
                    <div
                        ref={dropdownRef}
                        className="fixed z-[101] w-52 overflow-hidden rounded-xl border border-slate-100 bg-white py-1 shadow-xl animate-in fade-in zoom-in-95 duration-100"
                    ...
                    >
                        ...
                        <button
                            ...
                            onClick={() => {
                                setIsOpen(false);
                                router.push(`/ecommerce/customers/${customer.id}`);
                            }}
                            className="flex w-full items-center gap-2.5 px-3.5 py-2 text-left text-xs font-semibold text-primary hover:bg-primary/5 transition"
                        >
                            <ExternalLink className="h-4 w-4 text-primary" />
                            <span>{t('ecomm_customers_view_profile') || 'View Profile'}</span>
                        </button>
                        ...
                        >
                            ...
                            <span>{t('ecomm_customers_quick_preview') || 'Quick Preview'}</span>
                        </button>
                        ...
                        <button
                            ...
                            onClick={() => {
                                setIsOpen(false);
                                router.push(`/ecommerce/orders/create?customer_id=${customer.id}`);
                            }}
                        ...
                        >
                            ...
                            <span>{t('ecomm_create_order') || 'Create Order'}</span>
                        </button>

                        {/* Call Customer if mobile exists */}
                        ...
                        {/* Mail Customer if email exists */}
                        {customer.email && (
                            <a
                                href={`mailto:${customer.email}`}
                                onClick={() => setIsOpen(false)}
                                className="flex w-full items-center gap-2.5 px-3.5 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 transition border-t border-slate-50 truncate"
                            >
                                        ...
                                        return (
                                            <tr key={cust.id} className="hover:bg-slate-50/60 transition group">
                                                ...
                                                <td className="px-4 py-3.5 text-right">
                                                    <EcommerceCustomerActions
                                                        customer={cust}
                                                        onQuickView={setQuickViewCustomerId}
                                                    />
                                                </td>
                                            </tr>
                                        );
