...
const SerialSelectionModal = ({ isOpen, onClose, product, selectedStock, onConfirm }: SerialSelectionModalProps) => {
    ...
    const handleConfirm = () => {
        if (selectedSerialIds.length !== quantity) {
            setError(`${t('msg_select_exactly')} ${formatNumber(quantity)} ${t('lbl_serial_numbers')}`);
            return;
        }
        setError(null);
                            ...
                            <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-xl bg-white text-left align-middle shadow-2xl transition-all sm:rounded-2xl">
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm text-gray-500">
                                                {t('lbl_available')}: <span className="font-semibold text-gray-700">{formatNumber(availableSerials.length)}</span> {t('lbl_serials')}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex gap-3 border-t border-primary/10 bg-gradient-to-r from-primary/5 to-white px-4 py-4 sm:justify-end sm:px-6">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-primary/90 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 sm:flex-none"
                                    >
