        ...
        <div className="min-h-screen bg-gray-50 print:bg-white print:p-0 sm:bg-gray-100">
            <div className="invoice-shell mx-auto max-w-5xl bg-white shadow-lg print:shadow-none sm:my-0">
                ...
                <div className="sticky top-0 z-10 border-b border-gray-200 bg-white shadow-sm print:hidden">
                    {/* Primary row: Close + actions */}
                    <div className="flex items-center gap-2 px-3 py-2 sm:px-4">
                        {onClose ? (
                            <button
                                onClick={onClose}
                                className="flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100"
                            >
                                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                                </svg>
                                <span className="hidden xs:inline sm:inline">{t('btn_close')}</span>
                            </button>
                        ) : (
                            <div />
                        )}
                        <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
                            <button
                                onClick={handlePrint}
                                disabled={isPrinting}
                                title={t('btn_print_invoice')}
                                className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-50 sm:px-3"
                            >
                                <svg className="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                </svg>
                                <span className="hidden sm:inline">{isPrinting && printMode === 'invoice' ? t('status_opening') : t('btn_print_invoice')}</span>
                            </button>
                            <button
                                onClick={() => printWithMode('receipt')}
                                disabled={isPrinting}
                                title={t('btn_print_receipt')}
                                className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-2.5 py-2 text-sm font-medium text-white hover:bg-emerald-700 disabled:opacity-50 sm:px-3"
                            >
                                <svg className="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                <span className="hidden sm:inline">{isPrinting && printMode === 'receipt' ? t('status_opening') : t('btn_print_receipt')}</span>
                            </button>
                            <button
                                onClick={exportPDF}
                                disabled={isPrinting}
                                title={t('btn_download_pdf')}
                                className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-2.5 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 sm:px-3"
                            >
                                <svg className="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                <span className="hidden sm:inline">{isPrinting && !printMode ? t('status_generating') : t('btn_download_pdf')}</span>
                            </button>
                        </div>
                    </div>
                    {/* Mobile label row — shows button names below icons */}
                    <div className="grid grid-cols-3 border-t border-gray-100 sm:hidden">
                        <button
                            onClick={handlePrint}
                            disabled={isPrinting}
                            className="flex flex-col items-center gap-0.5 py-1.5 text-center text-[10px] font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50"
                        >
                            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                            </svg>
                            {isPrinting && printMode === 'invoice' ? t('status_opening') : t('btn_print_invoice')}
                        </button>
                        <button
                            onClick={() => printWithMode('receipt')}
                            disabled={isPrinting}
                            className="flex flex-col items-center gap-0.5 border-x border-gray-100 py-1.5 text-center text-[10px] font-medium text-emerald-700 hover:bg-emerald-50 disabled:opacity-50"
                        >
                            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            {isPrinting && printMode === 'receipt' ? t('status_opening') : t('btn_print_receipt')}
                        </button>
                        <button
                            onClick={exportPDF}
                            disabled={isPrinting}
                            className="flex flex-col items-center gap-0.5 py-1.5 text-center text-[10px] font-medium text-blue-700 hover:bg-blue-50 disabled:opacity-50"
                        >
                            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            {isPrinting && !printMode ? t('status_generating') : t('btn_download_pdf')}
                        </button>
                    </div>
                </div>
                ...
                {/* Success Message */}
                {isOrderCreated && (
                    <div className="border-b border-green-200 bg-green-50 px-4 py-3 print:hidden">
                        <div className="flex items-center gap-2 text-green-800">
                            <svg className="h-5 w-5 shrink-0 text-green-600 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <div>
                                <p className="text-sm font-semibold sm:text-lg">{t('msg_order_created_successfully')}</p>
                                <p className="text-xs text-green-700 sm:text-sm">{t('msg_order_processed_saved')}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Invoice Content */}
                <div className="p-3 sm:p-6 print:p-4" ref={invoiceRef}>
                    {/* Header with Logo */}
                        ...
                        <div className="flex items-start gap-3 sm:gap-6">
                            ...
                            <div className="min-w-0 flex-grow">
                                ...
                                <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-gray-600 sm:mt-2 sm:gap-4 sm:text-sm">
                                    ...
                                    {currentStore?.store_email && currentStore?.store_contact && <span className="hidden sm:inline">|</span>}
                                    {currentStore?.store_contact && <span>{t('lbl_phone')}: {currentStore.store_contact}</span>}
                                </div>
                                {registrationNumber && (
                                    <p className="mt-0.5 text-xs font-semibold text-gray-700 sm:mt-1 sm:text-sm">{registrationLabel}: {registrationNumber}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-3 text-center sm:mb-4">
                        <h2 className="text-xl font-bold text-gray-800 sm:text-2xl">{isReturn ? t('lbl_return_receipt') : t('lbl_invoice')}</h2>
                    </div>

                    {/* Invoice Details and Customer Info */}
                    <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div className="mb-4 flex justify-end">
                        <div className="w-full text-sm sm:w-80">
                            {!isReturn ? (
                                <>
