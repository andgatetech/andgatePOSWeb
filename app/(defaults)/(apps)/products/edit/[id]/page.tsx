...
const ProductEditForm = () => {
    ...
    const { data: productResponse, isLoading: productLoading } = useGetSingleProductQuery(productId);
        ...
        return (
            <div className="flex min-h-screen items-center justify-center">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push('/products')} className="mt-4 rounded-lg bg-blue-600 px-4 py-2 text-white">
                        Back to Products
                    </button>
                </div>
            </div>
                ...
                <div className="mb-8 rounded-2xl bg-white p-6 shadow-sm transition-shadow duration-300 hover:shadow-sm">
                    <div className="mb-6 flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            <button onClick={() => router.push('/products')} className="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100 transition-colors hover:bg-gray-200">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">{currentStore ? `Update product in ${currentStore.store_name}` : 'Update product details'}</p>
                            </div>
                        </div>
                    </div>
                    {currentStore && (
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                                                        {imageList.map((image, index) => (
                                                            <div key={index} className="group relative">
                                                                ...
                                                                <div className="absolute left-2 top-2 rounded-full bg-green-600 bg-opacity-90 px-2 py-1 text-xs text-white">New</div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
